...
| `polyomics-041e5834-validation.md` | generated by `python -m llm4pol.data validate` on the pinned PolyOmics snapshot (revision `041e5834…`, Phase 2, 2026-09-22); the number authority for every PolyOmics figure cited in this repository (charter §10); regenerated, never hand-edited. **Republished 2026-09-24 under ADR-0006**, which resolves a missing `tacticity` from its labelled twin before the candidate identity is taken, so its candidate-level numbers differ from the 2026-09-22 generation; row-level numbers are unchanged |
