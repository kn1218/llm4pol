...
#### Correction (2026-09-22, Phase 2 validator)

The number authority for this snapshot is now `docs/audit/polyomics-041e5834-validation.md`,
generated by `python -m llm4pol.data validate` on the pinned revision `041e5834…`. The
2026-09-11 text above is kept as written; the paragraphs below correct it.

`static_dielectric_const` is not a broken column: it is the uncorrected orientational
permittivity of non-polarisable MD, and `dielectric_const_dc = static_dielectric_const − 1 +
refractive_index²` holds on all 93,488 rows (max residual < 1e-5; arXiv:2511.11626 Table S3
calls it the corrected static dielectric constant). "Zero Maxwell violations" for
`dielectric_const_dc` is therefore an identity, not a check. A-7 stands: the registry serves
`dielectric_const_dc` only.

The 88.9 % is 88.82 % (38,693 / 43,561) on the triple rows and 84.30 % (78,807 / 93,488) on all
rows carrying both columns. The rank correlation −0.05 did not reproduce: 0.003 on all rows,
−0.144 on the triple rows.

The 43,561 is exactly `thermal_conductivity` non-null ∧ `dielectric_const_dc` ∈ [1, 20] ∧ `tg` ∈
[100, 900] K, with no `tg_rmse` term; the protocol `check_tc == True` filter gives 42,733
alongside.
