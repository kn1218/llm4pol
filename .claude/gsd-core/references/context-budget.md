...
The keys live in `.claude/settings.json` (project) or `C:/Users/molsim/Desktop/LLM4POL/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @C:/Users/molsim/Desktop/LLM4POL/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
